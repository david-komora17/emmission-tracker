...
const QuotaPaywallCard = ({ errorDetails, onClose, onPaymentSuccess, onRetryAI }) => {
    ...
    const startPolling = (targetCheckoutId) => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (attempts >= maxAttempts) {
                        clearInterval(intervalRef.current);
                        setPollingStatus('failed');
                        setPollingError('Payment is still pending. Please confirm the transaction on your handset or try again.');
                        return;
                    }
        ...
        // Start polling immediately, and continue every 3 seconds
            ...
            <div className="space-y-4">
                <div className="p-6 bg-green-50 border border-green-200 rounded-xl text-center">
                    ...
                    <div className="mt-3 text-sm text-gray-700">
                        ...
                        {pollingStatus === 'polling' && 'Pending approval on M-Pesa'}
                        {pollingStatus === 'completed' && 'Completed successfully'}
                        {pollingStatus === 'failed' && 'Failed or cancelled'}
                        {pollingStatus === 'idle' && 'Awaiting payment initiation'}
                    </div>
                    {renderPollingStatus()}
                    <button
                        onClick={onClose}
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 relative max-h-[calc(100vh-3rem)] overflow-hidden">
                <button
                    onClick={onClose}
